/**
 * @file main.cpp
 * @brief SDL3 + pass-mode simple-stress app bootstrap target.
 */
#include <array>
#include <algorithm>
#include <cstddef>
...
void record_stress_draw(varre::engine::PassCommandEncoder &encoder, const vk::Extent2D extent,
...
void build_stress_frame_graph(const varre::engine::PassFrameContext &frame_context, varre::engine::PassGraph *graph,
                              const std::span<const varre::engine::PassShaderBinding> shader_bindings, const varre::engine::GpuMesh &mesh,
                              const GpuDepthBuffer &depth_buffer) {
  if (graph == nullptr) {
    throw std::invalid_argument("build_stress_frame_graph requires a valid graph pointer.");
  }
  ...
  static_cast<void>(graph->add_phase(
    ...
    [shader_bindings, extent = frame_context.extent, &mesh](varre::engine::PassCommandEncoder &encoder) {
      record_stress_draw(encoder, extent, shader_bindings, mesh);
    }));
}
...
int main() {
  std::cout << "Starting varre-app simple-stress...\n";
  try {
    varre::app::AppCoreCreateInfo create_info{};
    create_info.window.title = "varre-app simple-stress";
    create_info.window.width = 1280;
    create_info.window.height = 720;

    create_info.engine.application_name = "varre-app-simple-stress";
    ...
    while (running) {
      ...
      const varre::engine::PassFrameRunResult result = app.pass_frame_loop().run_frame(
        ...
        [&mesh_shader_bindings, &mesh, &depth_buffer](const varre::engine::PassFrameContext &context, varre::engine::PassGraph *graph) {
          build_stress_frame_graph(context, graph, std::span<const varre::engine::PassShaderBinding>{mesh_shader_bindings}, mesh, depth_buffer);
        },
        run_info);
