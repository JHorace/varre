...
namespace varre::engine {
  namespace {
    ...
    void validate_binding_compatibility(const ReflectedDescriptorBinding &lhs, const ReflectedDescriptorBinding &rhs) {
      if (lhs.descriptor_type != rhs.descriptor_type || lhs.descriptor_count != rhs.descriptor_count) {
        throw std::runtime_error(
            fmt::format(
                "Incompatible descriptor binding reflection at set {} binding {}",
                lhs.set,
                lhs.binding
            )
        );
    ...
    void sort_binding_specs(std::vector<DescriptorBindingSpec> *bindings) {
      std::ranges::sort(
          *bindings,
          [](const DescriptorBindingSpec &lhs, const DescriptorBindingSpec &rhs) { return lhs.binding < rhs.binding; }
      );
    }
    ...
    std::vector<std::uint32_t> canonical_binding_words(const DescriptorSetLayoutSpec &spec) {
      ...
      std::ranges::sort(
          keys,
          [](const BindingKey &lhs, const BindingKey &rhs) { return lhs.binding < rhs.binding; }
      );
    ...
    std::vector<std::uint32_t> canonical_push_constant_words(const std::vector<vk::PushConstantRange> &ranges) {
      ...
      std::ranges::sort(
          ordered,
          [](const vk::PushConstantRange &lhs, const vk::PushConstantRange &rhs) {
  ...
  std::vector<ReflectedDescriptorBinding> reflect_descriptor_bindings(std::span<const varre::assets::ShaderAssetView> shaders) {
    ...
    for (const varre::assets::ShaderAssetView &shader : shaders) {
      ...
      for (const ReflectedDescriptorBinding &binding : bindings) {
        ...
        validate_binding_compatibility(it->second, binding);
  ...
  std::vector<DescriptorSetLayoutSpec> build_descriptor_set_layout_specs(
      std::span<const ReflectedDescriptorBinding> bindings,
      const vk::DescriptorSetLayoutCreateFlags create_flags
  ) {
    std::map<std::uint32_t, DescriptorSetLayoutSpec> by_set;
    for (const ReflectedDescriptorBinding &binding : bindings) {
      auto [it, inserted] = by_set.try_emplace(
          binding.set,
    ...
    for (auto &[set, spec] : by_set) {
      ...
      sort_binding_specs(&spec.bindings);
      out.push_back(std::move(spec));
    }
    return out;
  }

  DescriptorSetLayoutCache::DescriptorSetLayoutCache(const vk::raii::Device *device)
    : device_(device) {
  }

  DescriptorSetLayoutCache DescriptorSetLayoutCache::create(const EngineContext &engine) {
    return DescriptorSetLayoutCache{&engine.device()};
  }
  ...
  vk::DescriptorSetLayout DescriptorSetLayoutCache::get_or_create(const DescriptorSetLayoutSpec &spec) {
    ...
    const Key key{
      ...
      .binding_words = canonical_binding_words(spec),
    };
    ...
    const vk::DescriptorSetLayoutCreateInfo create_info = vk::DescriptorSetLayoutCreateInfo{}
        .setFlags(static_cast<vk::DescriptorSetLayoutCreateFlags>(key.create_flags))
  ...
  PipelineLayoutCache::PipelineLayoutCache(const vk::raii::Device *device)
    : device_(device) {
  }

  PipelineLayoutCache PipelineLayoutCache::create(const EngineContext &engine) {
    return PipelineLayoutCache{&engine.device()};
  }
  ...
  vk::PipelineLayout PipelineLayoutCache::get_or_create(const PipelineLayoutSpec &spec) {
    ...
    Key key{
      .create_flags = static_cast<std::uint32_t>(spec.create_flags),
      .set_layout_handles = {},
      .push_constant_words = canonical_push_constant_words(spec.push_constant_ranges),
    };
