 ...
 */
#include "varre/engine/material_descriptors.hpp"
...
namespace varre::engine {
  namespace {
     ...
     */
    std::vector<DescriptorSetLayoutSpec> densify_set_layout_specs(
        std::vector<DescriptorSetLayoutSpec> sparse_specs,
      ...
      std::ranges::sort(
          sparse_specs,
          [](const DescriptorSetLayoutSpec &lhs, const DescriptorSetLayoutSpec &rhs) { return lhs.set < rhs.set; }
      );
  ...
  MaterialPipelineDescriptorInterface::MaterialPipelineDescriptorInterface(
      DescriptorSetLayoutCache &&descriptor_set_layout_cache,
      PipelineLayoutCache &&pipeline_layout_cache
  )
    : descriptor_set_layout_cache_(std::move(descriptor_set_layout_cache)),
      pipeline_layout_cache_(std::move(pipeline_layout_cache)) {
  }

  MaterialPipelineDescriptorInterface MaterialPipelineDescriptorInterface::create(const EngineContext &engine) {
    return MaterialPipelineDescriptorInterface{
      DescriptorSetLayoutCache::create(engine),
      PipelineLayoutCache::create(engine),
    };
  }

  MaterialDescriptorLayout MaterialPipelineDescriptorInterface::get_or_create(const MaterialDescriptorRequest &request) {
    const std::vector<ReflectedDescriptorBinding> reflected_bindings = reflect_descriptor_bindings(request.shaders);
    std::vector<DescriptorSetLayoutSpec> sparse_specs = build_descriptor_set_layout_specs(
        reflected_bindings,
        request.descriptor_set_layout_create_flags
    );
    std::vector<DescriptorSetLayoutSpec> dense_specs = densify_set_layout_specs(
        std::move(sparse_specs),
        request.descriptor_set_layout_create_flags
    );
    const std::vector<vk::DescriptorSetLayout> set_layouts = descriptor_set_layout_cache_.get_or_create_all(dense_specs);

    const vk::PipelineLayout pipeline_layout = pipeline_layout_cache_.get_or_create(
        PipelineLayoutSpec{
  ...
  MaterialDescriptorLayout MaterialPipelineDescriptorInterface::get_or_create(const MaterialDescriptorShaderIdRequest &request) {
    ...
    for (const varre::assets::ShaderId shader_id : request.shader_ids) {
      ...
      if (shader == nullptr) {
        throw std::runtime_error(
            fmt::format(
                "Shader asset lookup failed for ShaderId value {}.",
                static_cast<std::uint32_t>(shader_id)
            )
        );
      }
  ...
  void MaterialPipelineDescriptorInterface::clear() {
    pipeline_layout_cache_.clear();
    descriptor_set_layout_cache_.clear();
  }

  std::size_t MaterialPipelineDescriptorInterface::descriptor_set_layout_cache_size() const noexcept {
    return descriptor_set_layout_cache_.size();
  }

  std::size_t MaterialPipelineDescriptorInterface::pipeline_layout_cache_size() const noexcept {
    return pipeline_layout_cache_.size();
  }
} // namespace varre::engine
