...
/**
 * @brief Backward-compatible alias for @ref SwapchainQueueTopology.
 */
using SurfaceQueueTopology = SwapchainQueueTopology;
...
public:
   ...
   */
  [[nodiscard]] const SwapchainQueueTopology &swapchain_queue_topology() const noexcept;

  /**
   * @brief Surface-resolved queue topology used by this swapchain.
   * @return Immutable queue topology.
   *
   * Compatibility alias for @ref swapchain_queue_topology.
   */
  [[nodiscard]] const SwapchainQueueTopology &queue_topology() const noexcept;
...
private:
  ...
  SwapchainContext(const EngineContext *engine, const SurfaceContext *surface_context, vk::SurfaceKHR surface, vk::raii::SwapchainKHR &&swapchain,
                   ...
                   vk::Extent2D extent, vk::PresentModeKHR present_mode, std::uint32_t max_frames_in_flight, SwapchainQueueTopology queue_topology,
  ...
  SwapchainQueueTopology swapchain_queue_topology_{};
...
using ::varre::engine::SurfaceQueueTopology;
using ::varre::engine::SwapchainContext;
using ::varre::engine::SwapchainCreateInfo;
using ::varre::engine::SwapchainQueueTopology;
} // namespace varre::engine::core
