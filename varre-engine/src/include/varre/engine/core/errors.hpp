 ...
 *
 * Ownership map:
 * - Core initialization: `kMissingRequirement`, `kNoSuitableDevice`, `kVulkanResult`
 * - Surface/swapchain: `kSurfaceUnsupported`, `kSwapchainOutOfDate`, `kSwapchainSuboptimal`
 * - Frame/sync flow: `kInvalidState`, `kInvalidArgument`, `kTimeout`
 * - Cross-cutting fallback: `kUnknown`
 */
...
using ::varre::engine::engine_error_code_name;
using ::varre::engine::EngineError;
using ::varre::engine::EngineErrorCode;
using ::varre::engine::is_recoverable_engine_error;
using ::varre::engine::make_engine_error;
using ::varre::engine::make_vulkan_result_error;
} // namespace varre::engine::core
